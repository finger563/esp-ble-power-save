...
#define TEST_DEVICE_NAME            "ESP_POWER_SAVE"
...
static esp_ble_adv_data_t adv_data = {
    ...
    .min_interval = 0x0012, //slave connection min interval, Time = min_interval * 1.25 msec
...
static esp_ble_adv_params_t adv_params = {
    .adv_int_min        = 0x0140,   // 15E0 3.5s / 1900 - 4s /1C20 -4.5s/1F40 -5s
    .adv_int_max        = 0x0140,
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param) {
    ...
    case ESP_GATTS_CONNECT_EVT: {
        ...
        for (int i=0; i<sizeof(conns); i++) {
            esp_err_t power_err = esp_ble_tx_power_set(conns[i], ESP_PWR_LVL_N12);
            ESP_LOGI(TAG, "TX_POWER_SET %d: %s", i, esp_err_to_name(power_err));
        }
        */
        esp_err_t power_err = esp_ble_tx_power_set(ESP_BLE_PWR_TYPE_DEFAULT, ESP_PWR_LVL_P9);
        ESP_LOGI(TAG, "TX_POWER_SET DEFAULT: %s", esp_err_to_name(power_err));
        esp_ble_conn_update_params_t conn_params = {0};
        memcpy(conn_params.bda, param->connect.remote_bda, sizeof(esp_bd_addr_t));
        uint8_t interval_15ms = 0x0c; // 15ms
        uint8_t interval_20ms = 0x10; // 20ms
        uint8_t interval_30ms = 0x18; // 30ms
        uint8_t interval_40ms = 0x20; // 40ms
        uint8_t interval_100ms = 0x50; // 100ms
        /* For the IOS system, please reference the apple official documents about the ble connection parameters restrictions. */
        conn_params.latency = 0;
        conn_params.max_int = interval_20ms;
        conn_params.min_int = interval_20ms;
